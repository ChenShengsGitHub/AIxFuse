...
def iter_gen(args,iter):
    ...
    if not args.single_process or args.ligpre:
        # 生成分子的ligpre
        ligpre(os.path.abspath(iter_csv),os.path.abspath(iter_mae),args.nchir,device=device)
    ...
    if not args.single_process or args.docking:
        # 生成分子的docking
        docking(os.path.abspath(iter_mae),os.path.abspath(grid_5ntp),prec=prec,path=os.path.abspath(docking_dir),device=device)
        time.sleep(10)
        docking(os.path.abspath(iter_mae),os.path.abspath(grid_6qu7),prec=prec,path=os.path.abspath(docking_dir),device=device)
        for i in trange(100000):
            time.sleep(10)
            if os.path.exists(mae_5ntp) and os.path.exists(mae_6qu7):
                break
        os.system('/opt/schrodinger2020-3/run python3 utils/mae_to_csv.py '
